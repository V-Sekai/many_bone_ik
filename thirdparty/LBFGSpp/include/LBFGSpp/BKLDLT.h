...
public:
    ...
    int compute(ConstGenericMatrix& mat, int uplo = Eigen::Lower, const Scalar& shift = Scalar(0))
    {
        using std::abs;

        m_n = mat.rows();
        if (m_n != mat.cols()) {
            std::cerr << "BKLDLT: matrix must be square" << std::endl;
            return -1;
        }
    ...
    // Solve Ax=b
    int solve_inplace(GenericVector b) const
    {
        if (!m_computed) {
            std::cerr << "BKLDLT: need to call compute() first" << std::endl;
            return -1;
        }
