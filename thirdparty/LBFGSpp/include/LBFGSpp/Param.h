...
public:
    ...
    /// is invalid.
    ///
    inline Error check_param() const
    {
        if (m <= 0)
        ...
        return OK; // Return OK if all parameters are valid
    }
};
...
public:
    ...
    {
        ERR_FAIL_COND_MSG(m <= 0, "'m' must be positive");
        ERR_FAIL_COND_MSG(epsilon < 0, "'epsilon' must be non-negative");
        ERR_FAIL_COND_MSG(epsilon_rel < 0, "'epsilon_rel' must be non-negative");
        ERR_FAIL_COND_MSG(past < 0, "'past' must be non-negative");
        ERR_FAIL_COND_MSG(delta < 0, "'delta' must be non-negative");
        ERR_FAIL_COND_MSG(max_iterations < 0, "'max_iterations' must be non-negative");
        ERR_FAIL_COND_MSG(max_submin < 0, "'max_submin' must be non-negative");
        ERR_FAIL_COND_MSG(max_linesearch <= 0, "'max_linesearch' must be positive");
        ERR_FAIL_COND_MSG(min_step < 0, "'min_step' must be positive");
        ERR_FAIL_COND_MSG(max_step < min_step, "'max_step' must be greater than 'min_step'");
        ERR_FAIL_COND_MSG(ftol <= 0 || ftol >= 0.5, "'ftol' must satisfy 0 < ftol < 0.5");
        ERR_FAIL_COND_MSG(wolfe <= ftol || wolfe >= 1, "'wolfe' must satisfy ftol < wolfe < 1");
    }
};
